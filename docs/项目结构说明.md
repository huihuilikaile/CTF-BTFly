...
- 状态：`ready`、`queued`、`delegating`、`provisioning`、`running`、`paused`、`settled`、`failed`、`cancelled`；其中 `queued` 表示等待全局并发名额，`delegating` 表示父任务正在处理子任务协作；
...
### `internal/agent/delegation.go` 与 `transfer.go`

当前已实现通用的受控专项协作：根 Agent 在完成初步判断后，可为边界清晰的辅助问题创建最多 3 个子 Agent，题型可为 Web、Crypto、Pwn、Reverse、Forensics 或 Misc。子 Agent 不可继续委派，根 Agent 始终负责原题的主线、验证和最终 Writeup。

```text
根 Agent 发现可独立的专项问题
→ 保存必要 Artifact
→ 写入 .cpi/subtasks/requests/*.json
→ CTF-BTFly 归档请求，创建对应题型的隔离子任务
→ 复制原题附件和请求明确列出的普通 Artifact 到子工作区
→ 根 Agent 保留原容器和 Pi 会话，立即继续主线；子 Agent 解题并生成报告/脚本
→ CTF-BTFly 关闭子容器，并回写到父任务 artifacts/subtasks/<handoffId>/
→ 平台通知根 Agent 复现、交叉验证并整合结果
```
...
当前已实现：单题一个根 Pi Agent，以及由 Go 调度器控制的通用主 Agent + 子 Agent 协作。根 Agent 可创建最多 3 个独立专项子任务，并在子任务运行期间继续自己的主线；子 Agent 的结果只作为证据回传，不能替代根 Agent 对最终 Flag 的验证。
...
flowchart TD
    ...
    Orchestrator --> Misc["Misc 子 Agent"]
...
1. 根 Agent 保持原容器、Pi 会话和主线分析职责；子 Agent 只处理经请求界定的辅助问题；
2. 每个根任务一生最多创建 3 个子 Agent，子 Agent 不得继续创建子任务；
3. 子任务只接收结构化请求、原题附件和明确列出的普通 Artifact，不传递完整聊天记录；
4. Go 调度器统一控制全局并发，父子组合不会绕过运行名额；
5. 每个子 Agent 使用独立工作区，完成后仅回传报告、脚本、Artifact 与结果清单。
