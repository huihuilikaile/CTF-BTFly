          ...
          {activeView === 'task' && selected ? (
            ...
            : activeView === 'security' ? <SecurityPolicyView system={system.data} />
              : activeView === 'usage' ? <ModelUsageView client={client!} system={system.data} tasks={tasks.data ?? []} />
                : activeView === 'info' ? <AboutView />
                  : <TaskListView client={client!} title={activeView === 'all' ? '全部题目' : activeView === 'running' ? '运行与排队任务' : `${viewTitle(activeView)}题目`} subtitle={activeView === 'all' ? '显示本机全部题目；可直接查看 Flag、复制或下载解题报告。' : activeView === 'running' ? '正在创建沙箱、执行 Pi Agent、等待子 Agent 回传、已暂停或等待执行名额的题目' : `仅显示“${viewTitle(activeView)}”方向的题目`} tasks={(tasks.data ?? []).filter(task => activeView === 'all' ? true : activeView === 'running' ? task.status === 'queued' || task.status === 'delegating' || task.status === 'running' || task.status === 'provisioning' || task.status === 'paused' : task.category === categoryForView(activeView))} onSelect={task => { setSelectedTaskID(task.id); setActiveView('task') }} onCreate={() => setShowCreate(true)} />}
        </main>
      </div>
...
// finalFlagFromWriteup 镜像后端规则，只接受“最终 Flag”章节的首个单行代码块。
...
function AboutView() {
  return <div className="panel-grid h-full overflow-y-auto p-6"><div className="mx-auto max-w-5xl">
    ...
    <section className="mt-4 rounded-lg border border-slate-800 bg-[#0d131b]/95 p-5"><div className="flex items-center gap-2 text-sm font-medium text-slate-200"><Info size={16} className="text-sky-300" />关于</div><dl className="mt-4 grid grid-cols-[112px_1fr] gap-y-2 text-xs"><dt className="text-slate-500">应用版本</dt><dd className="mono text-slate-300">0.1.0</dd><dt className="text-slate-500">默认后端地址</dt><dd className="mono text-slate-300">127.0.0.1:18731</dd><dt className="text-slate-500">数据位置</dt><dd className="text-slate-300">应用程序同目录的 <span className="mono">data/</span></dd><dt className="text-slate-500">致谢</dt><dd className="text-slate-300">Cnell: <a href="https://github.com/dlongx" target="_blank" rel="noopener noreferrer" className="mono text-sky-300 hover:underline">https://github.com/dlongx</a></dd><dt className="text-slate-500">关于我: </dt><dd className="text-slate-300">huihuilikaile: <a href="https://github.com/huihuilikaile" target="_blank" rel="noopener noreferrer" className="mono text-sky-300 hover:underline">https://github.com/huihuilikaile</a></dd></dl></section>
  </div></div>
}
